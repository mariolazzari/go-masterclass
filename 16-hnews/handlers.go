...
func (app *application) listPosts(w http.ResponseWriter, r *http.Request) {
	...
	posts, metadata, err := app.postRepo.GetAll(filter)
	if err != nil {
		app.errorLog.Printf("error getting posts: %s\n", err.Error())
		app.renderJSON(w, map[string]interface{}{
...
func (app *application) renderJSON(w http.ResponseWriter, data interface{}) {
	w.Header().Set("Content-Type", "application/json")
	bs, err := json.Marshal(data)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte(`{"error": "error rendering data"}`))
		return
	}
	w.Write(bs)
}
